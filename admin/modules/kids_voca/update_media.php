...
if($action == "execute"){
   if($fs_errorMsg == ""){
   	$myform->removeHTML(0);//lo?i b?  các ký t? html( 0 thi ko lo?i b?, 1: b?) tránh l?i
...
<?=template_top($fs_title)?>
   <p align="center" style="padding-left:10px;">
      ...
      $form->create_table();
      ?>
      <?=$form->getFile("T?i Audio", "toque_audio", "toque_audio", "T?i audio", 0, 30, "", "")?>
      <?=$form->getFile("T?i ?nh", "toque_image", "toque_image", "T?i ?nh", 0, 30, "", "")?>
      <?=$form->button("submit" . $form->ec . "reset", "submit" . $form->ec . "reset", "submit" . $form->ec . "reset", "C?p nh?t" . $form->ec . "Làm l?i", "C?p nh?t" . $form->ec . "Làm l?i", 'style="background:url(' . $fs_imagepath . 'button_1.gif) no-repeat"' . $form->ec . 'style="background:url(' . $fs_imagepath . 'button_2.gif)"', "");?>
