	...
	<table border="1" cellpadding="3" cellspacing="0" class="table" width="100%" bordercolor="<?=$fs_border?>">
   	<tr>
		...
		<input type="hidden" name="iQuick" value="update" />
         ...
         $n++;
         $dlUnut = '';
         $db_unit = new db_query("SELECT com_cou_id,com_name,com_id,com_num_unit,com_active,learn_wr_id,learn_wr_content
   					  	             FROM courses_multi,learn_writing
   						             WHERE courses_multi.com_id = learn_writing.learn_unit_id
         ...
         ?>
            ...
            if($cou_id == $rowCourse['cou_id']){
               ...
               $dlUnut .='></td>';
               $dlUnut .= '<td nowrap="nowrap" style="padding-left: 20px;height: 20px;line-height: 20px;">|------ <b style="color:red">Writing của Unit '.$rowUnit["com_num_unit"].' </b></td>';
               $dlUnut .= '<td align="center"><a title="Xem chi tiết" class="thickbox noborder a_detail" href="listdetail.php?url='.base64_encode(getURL()).'&record_id='.$rowUnit["learn_wr_id"].'&TB_iframe=true&amp;height=450&amp;width=950"">Xem chi tiết</a></td>';
               $dlUnut .= '<td align="center">'.$rowUnit["com_num_unit"].'</td>';
               $dlUnut .= '<td align="center"><a class="text" href="editgram.php?record_id='.$rowUnit["learn_wr_id"].'&returnurl='.base64_encode(getURL()).'"><img src="'.$fs_imagepath.'edit.png" alt="EDIT" border="0"></a></td>';
               $dlUnut .= '<td align="center"><img src="'.$fs_imagepath.'delete.gif" alt="DELETE" border="0" onClick="if (confirm(\'Are you sure to delete?\')){ window.location.href=\'deletegram.php?record_id='.$rowUnit["learn_wr_id"].'&returnurl='.base64_encode(getURL()).'\'}" style="cursor:pointer"></td>';
               $dlUnut .= '</tr>';
               }
               echo $dlUnut;
            }
