	...
	<table border="1" cellpadding="3" cellspacing="0" class="table" width="100%" bordercolor="<?=$fs_border?>">
   	<tr> 
		...
		<input type="hidden" name="iQuick" value="update" />	
         ...
         $m = 0;
         $n = 0;   
         $db_course = new db_query("SELECT cou_id,cou_cat_id,cou_name,cou_form FROM categories_multi
                                    INNER JOIN courses ON courses.cou_cat_id = categories_multi.cat_id
         ...
         $n++;
         $dlUnut = '';
         $db_unit = new db_query("SELECT com_cou_id,com_name,com_id,com_num_unit,com_active,les_det_id,les_det_content,les_det_name
                                  ...
                                  AND lesson_details.les_det_type = 1
         ...
         ?>
            ...
            if($cou_id == $rowCourse['cou_id']){
               ...
               $dlUnut .= '><td';
               $dlUnut .='></td>';
               $dlUnut .= '<td nowrap="nowrap" style="padding-left: 20px;height: 20px;line-height: 20px;">|------ <b style="color:red">Lesson main của Unit '.$rowUnit["com_num_unit"].' </b></td>';
               if($rowCourse["cou_form"] == 1 || $rowCourse["cou_form"] == 2){
                  $dlUnut .= '<td align="center"><a title="Xem chi tiết" class="thickbox noborder a_detail" href="listdetail.php?url='.base64_encode(getURL()).'&record_id='.$rowUnit["les_det_id"].'&TB_iframe=true&amp;height=500&amp;width=950"">Xem chi tiết [NORMAL & TOEFL]</a></td>';
               }elseif($rowCourse["cou_form"] == 3){
                  $dlUnut .= '<td align="center"><a title="Xem chi tiết" class="thickbox noborder a_detail" href="listdetail_1.php?url='.base64_encode(getURL()).'&record_id='.$rowUnit["les_det_id"].'&TB_iframe=true&amp;height=500&amp;width=950"">Xem chi tiết [TOEIC & IELTS]</a></td>';
               }
               $dlUnut .= '<td align=""></td>';
               $dlUnut .= '<td align="center">'.$rowUnit["com_num_unit"].'</td>';
               $dlUnut .= '<td align="center"><a class="text" href="edit.php?record_id='.$rowUnit["les_det_id"].'&returnurl='.base64_encode(getURL()).'"><img src="'.$fs_imagepath.'edit.png" alt="EDIT" border="0"></a></td>';
               $dlUnut .= '<td align="center"><img src="'.$fs_imagepath.'delete.gif" alt="DELETE" border="0" onClick="if (confirm(\'Are you sure to delete?\')){ window.location.href=\'delete.php?record_id='.$rowUnit["les_det_id"].'&returnurl='.base64_encode(getURL()).'\'}" style="cursor:pointer"></td>';
               $dlUnut .= '</tr>';
               }
               echo $dlUnut;
            }   
