...
checkAddEdit("add");
   ...
   $db_cate    =  new db_query('SELECT cat_name,cat_id,lev_name
                                 FROM categories_multi
                                 INNER JOIN courses
                                 ON(cou_cat_id = cat_id)
                                 INNER JOIN levels
   ...
   $iCate =  $dbcate['cat_id'];
   if($dbcate){
      $link = generate_preview_link($nCate,$dbcate['lev_name'],$nLesson,$nCourse,$iCoirse,'strategy');
   }
   ...
   $myform->removeHTML(0);
   $myform->add("main_det_id", "main_det_id", 1, 0,$record_id, 0, "", 0, "");
   ...
   $myform->add("main_media_type","main_media_type",1,0,0,0,translate_text("Bạn chưa chọn dạng media"),0,"");
   $myform->add("main_type","main_type",1,1,1,0);
    ...
    if($action == "execute"){
      ...
      if($fs_errorMsg == ""){
         ...
         if($fs_errorMsg == ""){
            ...
            unset($db_insert);       			
      		redirect("listdetail_1.php?url=".base64_encode(getURL())."&record_id=".$record_id);
      	}
      } 
   }
...
<?=$load_header?>
   ...
   <? /*------------------------------------------------------------------------------------------------*/ ?>
      ...
      <?=$form->radio("<font class='form_asterisk'>*</font> Chọn kiểu media","main_media_type","main_media_type",1,$main_media_type,"Ảnh",0,"","")?>
      ...
      <tr>
         <td colspan="2">
         <?=$form->wysiwyg("<font class='form_asterisk'>*</font> Nội dung tiếng anh", "main_content_en", $main_content_en, "../../resource/wysiwyg_editor/", 800, 500)?>
         </td>
      </tr>
      <tr>
         <td colspan="2">
         <?=$form->wysiwyg("<font class='form_asterisk'>*</font> Nội dung tiếng việt", "main_content_vi", $main_content_vi, "../../resource/wysiwyg_editor/", 800, 500)?>
         </td>
      </tr>
      ...
      <tr <? if($i%2==0) echo ' bgcolor="#FAFAFA"';?>>
         ...
         <input style="width:30px;background: #eee;text-align: center;" type="text" readonly="" value="<?=$row["main_order"]?>" />
         </td>
         <td align="center"><a class="text" href="editdetail_1.php?record_id=<?=$row["main_id"]?>&returnurl=<?=base64_encode(getURL())?>"><img src="<?=$fs_imagepath?>edit.png" alt="EDIT" border="0"></a></td>
         <td align="center"><img src="<?=$fs_imagepath?>delete.gif" alt="DELETE" border="0" onClick="if (confirm('Are you sure to delete?')){ window.location.href='deletedetail.php?record_id=<?=$row["main_id"]?>&returnurl=<?=base64_encode(getURL())?>'}" style="cursor:pointer"></td>
