...
if ($iQuick == 'update'){
	...
	if($record_id != ""){
		for($i=0; $i<count($record_id); $i++){
			...
			//Insert to database
			$myform->add("lib_cat_name","cat_name" . $record_id[$i],0,0,"",0,"",0,"");
			if($array_config["order"]==1) $myform->add("lib_cat_order","cat_order" . $record_id[$i],1,0,0,0,"",0,"");
			...
			if($array_config["image"]==1){
				...
				if ($upload_pic->file_name != ""){
					...
					$myform->add("lib_cat_picture","picture",0,1,"",0,"",0,"");
				}
				if ($upload_pic->file_name != ""){
					//Delete file
					delete_file($fs_table,$id_field,$record_id[$i],"pcat_picture",$fs_filepath);
					//Permision file
				}
				//Check Error!
