...
checkAddEdit("add");
   ...
   if($iTyp > 0){
      $db_check_type = new db_query("SELECT iety_id FROM ielt_type WHERE iety_ielt_i = ". $iTyp ." AND iety_type = 2");
   ...
   $arr_get_typ = array();
   $db_select_test = new db_query("SELECT * FROM ielts STRAIGHT_JOIN ielt_type ON ielt_id = iety_ielt_id WHERE iety_type = 2 GROUP BY iety_ielt_id ");
   $i = 0;
   while($row_a = mysqli_fetch_assoc($db_select_test->result)){
      $arr_get_typ[$i] = $row_a["iety_ielt_id"];
      $i++;
   }
   ...
   $myform->add("iety_ielt_id", "iety_ielt_id", 1, 0, 0, 1, "Bạn chưa chọn đề thi", 0, "");
   $myform->add("iety_name", "iety_name", 0, 0, "", 1, "Bạn chưa nhập tên phần thi", 0, "");
   $myform->add("iety_type", "iety_type", 1, 1, 2, 0, "", 0, "");
...
<? /*------------------------------------------------------------------------------------------------*/ ?>
   ...
   $form->create_table();
	?>
   <?=$form->text_note('<strong style="text-align:center;">----------Thêm mới Test Listening-----------</strong>')?>
   ...
   <?=$form->text("Tên phần thi", "iety_name", "iety_name", "Listening", "Phần thi", 1, 272, "", 255, "", "", "")?>
