...
class sendMail{
	...
	function send($message, $addressEmail, $addressName, $subject = '', $from = '', $password = '', $reply = ''){
	    ...
	    //from.
	    if($from == ''){
	    	$this->mail->Username = 'eonline.hochay.vn@gmail.com'; //hss.svnn.analytics@gmail.com
			$this->mail->Password = 'hochay.vn'; //8g#yRW^Dhk4qes$AfYzr
			$this->mail->SetFrom('eonline.hochay.vn@gmail.com', "hochay.vn");
		}
		else{
			$this->mail->Username = $from;
			$this->mail->Password = $password;
			$this->mail->SetFrom($from, "hochay.vn");
		}

		//title.
		if($subject == ''){
			$subject = 'Kết quả chấm bài từ hochay.vn';
		}

		//Reply.
		if($reply == ''){
			$reply = 'eonline.hochay.vn@gmail.com';
		}
		...
		$this->mail->AddReplyTo($reply, "hochay.vn");
