...
*/
$list->add("listarm_img","Ảnh đại diện","string",0,0);
$list->add($name_field,"Mô tả","string", 1, 1);
$list->add("listarm_exp","EXP","string", 1, 1);
...
$db_listing 	= new db_query("SELECT * FROM " . $fs_table .
										   ...
										   . " ORDER BY " . $list->sqlSort() . "listarm_id DESC "
...
<div id="listing">
   ...
   while($row	=	mysqli_fetch_assoc($db_listing->result)){
     ...
     <td align="center">
            <img style="height: 40px;" src="<?=$imgpath . "small_" . $row['listarm_img'] ?>" />
        </td>
    ...
    <td class="bold" align="center">
      <?=$row['listarm_exp']?>
   	...
   	<?=$list->showEdit($row['listarm_id'])?>
   	<?=$list->showDelete($row['listarm_id'])?>
   	<?=$list->end_tr()?>
  	<?
   }
   ?>
