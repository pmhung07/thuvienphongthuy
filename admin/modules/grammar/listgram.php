      ...
      $j = 0;
		while($row = mysql_fetch_array($db_lesson->result)){ $i++;
         ...
         <tr <? if($i%2==0) echo ' bgcolor="#FAFAFA"';?>>
            ...
            <td nowrap="nowrap">
               <b><a class="thickbox noborder a_detail" href="listdetail.php?url=<?=base64_encode(getURL())?>&record_id=<?=$row["les_det_id"]?>&TB_iframe=true&amp;height=450&amp;width=950">Phần Grammar của Lesson : <?php echo $row['com_name'] ?></a></b>       
            </td>
            ...
            <td align="center" width="16"><a class="text" href="editgram.php?record_id=<?=$row["les_det_id"]?>&returnurl=<?=base64_encode(getURL())?>"><img src="<?=$fs_imagepath?>edit.png" alt="EDIT" border="0"></a></td>
            <td align="center"><img src="<?=$fs_imagepath?>delete.gif" alt="DELETE" border="0" onClick="if (confirm('Are you sure to delete?')){ window.location.href='deletegram.php?record_id=<?=$row["les_det_id"]?>&returnurl=<?=base64_encode(getURL())?>'}" style="cursor:pointer"></td>
         </tr>
		<? } unset($db_lesson) ?>
		</form>
	</table>
