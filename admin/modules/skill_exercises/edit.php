...
//Get Content id
$iCont = "";
$iCont = getValue("iCont","int","GET",0);
...
<?=template_top(translate_text("Edit member"))?>
   ...
   <form action="" onsubmit="searchKeyword(); return false;">
      <table>
         <tr>
            <td class="form_name">Tìm kiếm bài học :</td>
            <td>
               ...
               <p class="p_note">
                  ...
                  • Chọn Content : Chọn [Content] trước khi tạo [Quiz] <br/>
               </p>
            </td>
         </tr>
      </table>
   ...
   <p align="center" style="padding-left:10px;">
      ...
      ?>
         ...
         <tr>
            <td align="right"><font color="red">*</font>&nbsp;Chọn Content: </td>
            <td>
               <select class="form_control" name="cont_select" id="cont_select">
               <?
               foreach($arr_get_content as $id=>$name) {
                  echo('<option '. (($id == $iCont) ? 'selected="selected"' : '') .' value="'. $id .'">'. $name .'</option>');
               }
               ?>
               </select>
            </td>
