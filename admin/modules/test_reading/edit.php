...
//Call class menu - lay ra danh sach Category
   ...
   $myform->add("typ_type", "typ_type", 1, 0, 1, 0, "", 0, "");
...
<? /*------------------------------------------------------------------------------------------------*/ ?>
   ...
   $form->create_table();
   ?>
   <?=$form->text_note('<strong style="text-align:center;">----------Sửa đổi Test Reading-----------</strong>')?>
      ...
      <td>
         <select class="form_control" style="width: 186px;" name="typ_test_id" id="typ_test_id">
            ...
            <?foreach($arr_get_test as $id=>$name){?>
      			<option value="<?=$id?>"<?=($id == $typ_test_id) ? "selected='selected'" : ""?>><?=$name?></option>
      		<?}?>
         </select>
      </td>
   </tr>*/?>
   <?=$form->text("Tên phần thi", "typ_name", "typ_name", $typ_name, "Phần thi", 1, 272, "", 255, "", "", "")?>
