...
$list->add("cou_name","Course","string",1,1);
...
<div id="listing">
   ...
   while($row	=	mysql_fetch_assoc($db_listing->result)){
    ...
    $str_cate = "";
      ...
      //
      $url = 'http://'.$_SERVER['HTTP_HOST'].'/khoa-hoc/'.removeTitle($str_cate).'/' . removeTitle($row['cou_name'])	.	'/' . $row['cou_id'] . '-' . $row['com_id'] . '-speak.html'; 
   ?>    
      ...
      <td align="center" width="200">
         <input style="text-align: center;color: red;font-weight: bold;font-size: 11px;width: 250px;" value="<?=$row['com_name']?>" type="text" />
      </td>
      <td align="center" width="200">
      ...
      <td width="50" align="center">
         ...
         <button id="bt_send_mail" type="button" style="width: 76px;background: #EEE;text-align: center;font-weight: bold;color: red;cursor: pointer;" name="<?=$row['lsr_id']?>">Send Mail</button>
         <?php } ?>
      </td>
      <td align="center" width="15">
   		<a href="edit.php?record_id=<?=$row['lsr_id']?>&user_id=<?=$row['lsr_use_id']?>&les_name=<?=$row['com_name']?>&les_url=<?=$url?>&url=<?=base64_encode($_SERVER['REQUEST_URI'])?>" title="Bạn muốn sửa đổi bản ghi" rel="tooltip" class="edit">
