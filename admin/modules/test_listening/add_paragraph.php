...
if($action == "execute"){
   ...
   //up image
   $upload_img	= new upload("tec_image", $image_path, $fs_extension_img, $fs_filesize);
   $filename_img	= $upload_img->file_name;
   if($filename_img != ""){
   	$myform->add("tec_image","filename_img",0,1,0,0);
...
<? /*------------------------------------------------------------------------------------------------*/ ?>
   ...
   <?=$form->getFile("Tải Ảnh", "tec_image", "tec_image", "Tải ảnh", 0, 30, "", "")?>
	...
	<table cellpadding="5" cellspacing="0" width="100%" style="border-collapse:collapse;" bordercolor="#CCCCCC" border="1">
		...
		?>
			<tr <?=$fs_change_bg?>>
               ...
               <input style="width: 540px;background: #eee;" type="text" readonly="" value="<?=$row["tec_name"]?>" />
            </td>
            ...
            <td align="center" width="110">
               <img height="23px" src="<?=$image_path."small_".$row["tec_image"]?>" />
            </td>
