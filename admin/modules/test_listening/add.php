...
checkAddEdit("add");
   ...
   if($iTyp > 0){
      $db_check_type = new db_query("SELECT typ_id FROM type_test WHERE typ_test_id = ". $iTyp ." AND typ_type = 2");
   ...
   $arr_get_typ = array();
   $db_select_test = new db_query("SELECT * FROM test STRAIGHT_JOIN type_test ON test_id = typ_test_id WHERE typ_type = 2 GROUP BY typ_test_id ");
   $i = 0;
   ...
   $myform->add("typ_type", "typ_type", 1, 1, 2, 0, "", 0, "");   
...
<? /*------------------------------------------------------------------------------------------------*/ ?>
   ...
   <?=$form->text("Tên phần thi", "typ_name", "typ_name", "Listening", "Phần thi", 1, 272, "", 255, "", "", "")?>
