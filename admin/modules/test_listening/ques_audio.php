...
$myform->add("teque_time_audio", "total_audio_time", 1, 1, 0, 0, "", 0, "");
...
if($action == "execute"){      
   if($fs_errorMsg == ""){    	
      ...
      $upload		= new upload("teque_audio", $data_path, $fs_extension, $fs_filesize);
      $filename	= $upload->file_name;
      if($filename != ""){
         $myform->add("teque_audio","filename",0,1,0,0);
      }
      /*
      $upload_img	= new upload("teque_image", $image_path, $fs_extension_img, $fs_filesize);
      $filename_img	= $upload_img->file_name;
      if($filename_img != ""){
      	$myform->add("teque_image","filename_img",0,1,0,0);
...
if($teque_time_audio > 0){
   $tec_audio_second = $teque_time_audio	 % 60;
   $tec_audio_minute = ($teque_time_audio	 - $tec_audio_second)/60;
}else{
   $tec_audio_second = 0;
   $tec_audio_minute = 0;
}
...
<?=template_top($fs_title)?>
   <p align="center" style="padding-left:10px;">
      ...
      $form->create_table();
      ?>
      <?=$form->getFile("Tải Audio", "teque_audio", "teque_audio", "Tải audio", 0, 30, "", "")?>
      <tr>
      ...
      <?//=$form->getFile("Tải Ảnh", "teque_image", "teque_image", "Tải ảnh", 0, 30, "", "")?>
	...
	<table cellpadding="5" cellspacing="0" width="450px" style="border-collapse:collapse;" bordercolor="#CCCCCC" border="1">
      ...
      $db_picture = new db_query("SELECT * FROM test_questions WHERE  teque_id=".$iQues);
		...
		?>
            ...
            <td align="center" width="200"><b><?=$row["teque_time_audio"]?></b> s</td>
