...
if($iParent>0){
   $db_Course = new db_query("SELECT cou_id,cou_name FROM categories_multi
                              INNER JOIN courses
                              ON courses.cou_cat_id = categories_multi.cat_id
...
<div id="listing">
   ...
   <table border="1" cellpadding="3" cellspacing="0" class="table" width="100%" bordercolor="<?=$fs_border?>">
      ...
      if($cou_id == $rowCourse['cou_id']){
         ...
         $dlUnut .='></td>';
         $dlUnut .= '<td nowrap="nowrap" style="padding-left: 20px;height: 20px;line-height: 20px;">|------ <b style="color:red">Vocabulary của Unit '.$rowUnit["com_num_unit"].' </b></td>';
         $dlUnut .= '<td align="center"><a title="Xem chi tiết" class="thickbox noborder a_detail" href="listdetail.php?url='.base64_encode(getURL()).'&record_id='.$rowUnit["les_det_id"].'&TB_iframe=true&amp;height=450&amp;width=950"">Xem chi tiết</a></td>';
         $dlUnut .= '<td align="center"></td>';
         $dlUnut .= '<td align="center">'.$rowUnit["com_num_unit"].'</td>';
         $dlUnut .= '<td align="center"><a class="text" href="editvoca.php?record_id='.$rowUnit["les_det_id"].'&returnurl='.base64_encode(getURL()).'"><img src="'.$fs_imagepath.'edit.png" alt="EDIT" border="0"></a></td>';
         $dlUnut .= '<td align="center"><img src="'.$fs_imagepath.'delete.gif" alt="DELETE" border="0" onClick="if (confirm(\'Are you sure to delete?\')){ window.location.href=\'deletevoca.php?record_id='.$rowUnit["les_det_id"].'&returnurl='.base64_encode(getURL()).'\'}" style="cursor:pointer"></td>';
         $dlUnut .= '</tr>';
         }echo $dlUnut;
      }   
      ?>                
