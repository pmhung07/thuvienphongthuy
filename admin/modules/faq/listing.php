...
$db_parent = new db_query("SELECT que_id,que_content
                             FROM ".$fs_table."
                             WHERE 1
...
*/
$list->add($name_field,"Tên câu hỏi","string", 1, 1);
$list->add("ans_content","Câu trả lời","string",1,1);
$list->add("que_active","Active","int", 0, 0);
...
<div id="listing">
   ...
   ?>
      ...
      <td width="300" align="center">
         <?if($row['ans_content']){
            $type = 'del_answers';
            $type_act = 'act_answers';
            $record_id = $row['ans_id'];
            $act_action = $row['ans_active'];
            $fs_edit    = 'edit_answers.php';
            ?>
            <textarea style="width: 400px;height: 25px;"><?=truncateString_(removeHTML(($row['ans_content'])),120)?></textarea>
         <?}else{
            $type ='del_question';
            $type_act = 'act_question';
            $record_id = $row['que_id'];
            $act_action = $row['que_active'];
            $fs_edit    = 'edit_question.php';
            ?>
