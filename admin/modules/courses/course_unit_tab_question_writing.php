...
$fs_title = $module_name . " | Thêm nội dung cho phần bài tập dạng Writing ( Tabs )";
...
$db_block = new db_query("SELECT * FROM courses_multi_tabs_block WHERE  com_block_tab_id=" . $iTab . " AND com_block_data_type ='question_writing'");
$arrayBlock = array();
...
<? /*------------------------------------------------------------------------------------------------*/ ?>
	...
	<table cellpadding="5" cellspacing="0" width="100%" style="border-collapse:collapse;" bordercolor="#CCCCCC" border="1">
		...
		<?
		$sql = '';
		$db_picture = new db_query("SELECT * FROM courses_multi_tabs_block WHERE  com_block_tab_id=" . $iTab . " AND com_block_data_type ='question_writing'");
		?>
		...
		?>
			...
			<tr style="background:rgb(179, 218, 200);border-bottom: solid 1px white;">
	            ...
	            <td width="100" align="center">
	            	<a style="padding:5px 0px 5px 6px;text-decoration:none;" title="Add audio" class="thickbox noborder a_detail" href="course_unit_tab_question_writing_edit.php?iQues=<?=$value['cou_tab_question_id']?>'&url=<?=base64_encode(getURL())?>'&TB_iframe=true&amp;height=350&amp;width=1000">
	                  <b style="background: none repeat scroll 0 0;color: black;padding: 2px 10px;">Update Câu hỏi này</b>
	               </a>
	            </td>
	            <td align="center"><a onclick="if (confirm('Bạn muốn xóa bản ghi?')){ window.location.href='course_unit_tab_question_writing_delete.php?record_id=<?=$value["cou_tab_question_id"]?>&url=<?=base64_encode($_SERVER['REQUEST_URI'])?>' }" href="#" class="delete"><img border="0" src="<?=$fs_imagepath?>delete.gif"></a></td>
			</tr>
			<? } } ?>
		<? } ?>
	</table>
...
function addblockintab(tab_id){
	...
	$.ajax({
		...
		data:{
			tab_id:tab_id,
			tab_type:"addblockintabqueswriting",
			blockname:blockname
      	},
		url:'ajax.php',
