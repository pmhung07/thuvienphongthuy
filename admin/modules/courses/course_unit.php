	...
	<table class="background: #C7DBFF;" cellpadding="5" cellspacing="0" width="100%" style="border-collapse:collapse;" bordercolor="#CCCCCC" border="1">
		...
		?>
			<tr <?=$fs_change_bg?>>
            	...
            	<td align="center" align="center">
            		<p><a class="moreptr" title="Ảnh đại diện" class="thickbox noborder" href="more_picture.php?coursesid=<?=$record_id?>&record_id=<?=$row["com_id"]?>&TB_iframe=true&amp;height=450&amp;width=500" style="color:#0000FF; font-family:Tahoma; font-size:11px">Ảnh đại diện</a></p>
            	</td>
            	<td align="center">
			...
			<tr class="trinvi trinvi_<?=$row["com_id"]?>" style="background:rgb(221, 223, 255);">
            	...
            	<td align="center"><a onclick="if (confirm('Bạn muốn xóa bản ghi?')){ window.location.href='course_unit_tab_delete.php?record_id=<?=$rowtab["cou_tab_id"]?>&url=<?=base64_encode($_SERVER['REQUEST_URI'])?>' }" href="#" class="delete"><img border="0" src="<?=$fs_imagepath?>delete.gif"></a></td>
			</tr>
			<? } ?>
		<? } ?>
	</table>
