...
$fs_title = $module_name . " | Thêm nội dung cho phần bài tập dạng Multiple choice ( Tabs )";
...
$cou_tab_question_type = "multiplechoice";
...
if($action == "execute"){
	...
	if($fs_errorMsg == ""){
		$myform->removeHTML(0);
		$db_insertpack = new db_execute_return();
		$last_id_pack    = $db_insertpack->db_execute($myform->generate_insert_SQL());
		$trueans = getValue('answer','int','POST',0);
		for($i = 1; $i <= 5;$i++){
			$myformans = new generate_form();
			if($i == $trueans){
				$cou_tab_answer_true = 1;
			}else{
				$cou_tab_answer_true = 0;
			}
			if($_POST['cou_tab_answer_content_'.$i] != ""){
				$anscontent = $_POST['cou_tab_answer_content_'.$i];
				$myformans->add("cou_tab_answer_content", "anscontent", 0, 1, "", 0, "", 0, "");
				$myformans->add("cou_tab_answer_true", "cou_tab_answer_true", 1, 1, "", 0, "", 0, "");
				$myformans->add("cou_tab_answer_question_id", "last_id_pack", 1, 1, "", 0, "", 0, "");
				$myformans->addTable("courses_multi_tab_answers");
				$db_insertpacks = new db_execute_return();
				$last_id_packs    = $db_insertpacks->db_execute($myformans->generate_insert_SQL());
				unset($myformans);
			}
		}
...
$db_block = new db_query("SELECT * FROM courses_multi_tabs_block WHERE  com_block_tab_id=" . $iTab . " AND com_block_data_type ='question_multiplechoice'");
$arrayBlock = array();
...
<? $myform->checkjavascript();?>
	...
	<?=$form->text("Nội dung câu hỏi", "cou_tab_question_content", "cou_tab_question_content", "", "Nội dung câu hỏi", 0, 489, 20, 255, "", "", "")?>
	<?//=$form->text("Đoạn văn nếu có", "cou_tab_question_paragraph", "cou_tab_question_paragraph", "", "Tiêu đề", 0, 489, 20, 255, "", "", "")?>
	<tr>
		<td class="form_name">Đoạn văn nếu có :</td>
		<td class="form_text">
			<textarea class="cou_tab_cont_text" id="cou_tab_question_paragraph" name="cou_tab_question_paragraph" style="width:500px;height: 50px;"></textarea>
	...
	<?=$form->getFile("Upload Media Nếu có", "cou_tab_question_media", "cou_tab_question_media", "Upload Media", 1, 30, "style='border:none;'", "")?>
	<tr>
		<td class="form_name">Nội dung câu trả lời A :</td>
		<td class="form_text">
			<input class="form_control" type="text" title="Nội dung câu hỏi" id="cou_tab_answer_content_1" name="cou_tab_answer_content_1" value="" style="width:489px; height:20px" maxlength="255">
			<input type="radio" name="answer" value="1"><br>
		</td>
	</tr>
	<tr>
		<td class="form_name">Nội dung câu trả lời B :</td>
		<td class="form_text">
			<input class="form_control" type="text" title="Nội dung câu hỏi" id="cou_tab_answer_content_2" name="cou_tab_answer_content_2" value="" style="width:489px; height:20px" maxlength="255">
			<input type="radio" name="answer" value="2"><br>
		</td>
	</tr>
	<tr>
		<td class="form_name">Nội dung câu trả lời C :</td>
		<td class="form_text">
			<input class="form_control" type="text" title="Nội dung câu hỏi" id="cou_tab_answer_content_3" name="cou_tab_answer_content_3" value="" style="width:489px; height:20px" maxlength="255">
			<input type="radio" name="answer" value="3"><br>
		</td>
	</tr>
	<tr>
		<td class="form_name">Nội dung câu trả lời D :</td>
		<td class="form_text">
			<input class="form_control" type="text" title="Nội dung câu hỏi" id="cou_tab_answer_content_4" name="cou_tab_answer_content_4" value="" style="width:489px; height:20px" maxlength="255">
			<input type="radio" name="answer" value="4"><br>
		</td>
	</tr>
	<tr>
		<td class="form_name">Nội dung câu trả lời E :</td>
		<td class="form_text">
			<input class="form_control" type="text" title="Nội dung câu hỏi" id="cou_tab_answer_content_5" name="cou_tab_answer_content_5" value="" style="width:489px; height:20px" maxlength="255">
			<input type="radio" name="answer" value="5"><br>
		</td>
	</tr>
...
<? /*------------------------------------------------------------------------------------------------*/ ?>
	...
	<table cellpadding="5" cellspacing="0" width="100%" style="border-collapse:collapse;" bordercolor="#CCCCCC" border="1">
		...
		<?
		$sql = '';
		$db_picture = new db_query("SELECT * FROM courses_multi_tabs_block WHERE  com_block_tab_id=" . $iTab . " AND com_block_data_type ='question_multiplechoice'");
		?>
...
function set_true_ans(ans_id,ques_id){
	$.ajax({
		type:'POST',
		dataType:'text',
		data:{
...
function save_question(ques_id){
   	var quesname = $('#ques_content_'+ques_id).val();
	$.ajax({
		type:'POST',
		dataType:'text',
		data:{
...
function order_ques(ques_id){
   	var order = $('#order_ques_'+ques_id).val();
	$.ajax({
		type:'POST',
		dataType:'text',
		data:{
...
function del_question(ques_id){
	$.ajax({
		type:'POST',
		dataType:'text',
		data:{
			ques_id:ques_id,
			tab_type:"del_question"
      	},
		url:'ajax.php',
		success:function(data){
			if($.trim(data) == 1){
...
function save_answers(ans_id){
   	var ansname = $('#ans_content_'+ans_id).val();
	$.ajax({
		type:'POST',
		dataType:'text',
		data:{
...
function addblockintab(tab_id){
	...
	$.ajax({
		...
		data:{
			tab_id:tab_id,
			tab_type:"addblockintabquesmultiplechoice",
			blockname:blockname
      	},
		url:'ajax.php',
		success:function(data){
