...
$fs_title = $module_name . " | Thêm nội dung cho phần bài tập dạng Matching ( Tabs )";
...
$myform->add("cou_tab_question_type", "cou_tab_question_type", 0, 0, "", 0, "", 0, "");
...
$db_block = new db_query("SELECT * FROM courses_multi_tabs_block WHERE  com_block_tab_id=" . $iTab . " AND com_block_data_type ='question_matching'");
$arrayBlock = array();
...
$arrayTypeQues = array(
	"-1" => "Chọn dạng bài tập",
	"matching" => "Điên từ vào chỗ trống",
	"draganddrop" => "Kéo thả"
);
...
<? $myform->checkjavascript();?>
	...
	<?=$form->select("Chọn dạng bài tập Matching", "cou_tab_question_type", "cou_tab_question_type", $arrayTypeQues, -1 ,"Chọn dạng bài tập matching",1,"498",1,0,"","")?>
...
<? /*------------------------------------------------------------------------------------------------*/ ?>
	...
	<table cellpadding="5" cellspacing="0" width="100%" style="border-collapse:collapse;" bordercolor="#CCCCCC" border="1">
		...
		<?
		$sql = '';
		$db_picture = new db_query("SELECT * FROM courses_multi_tabs_block WHERE  com_block_tab_id=" . $iTab . " AND com_block_data_type ='question_matching'");
		?>
		...
		?>
			...
			<tr style="background:rgb(179, 218, 200);border-bottom: solid 1px white;">
	            ...
	            <td width="50" align="center"><?=$value["cou_tab_question_order"]?></td>
	            <td width="100" align="center">
	            	<a style="padding:5px 0px 5px 6px;text-decoration:none;" title="Add audio" class="thickbox noborder a_detail" href="course_unit_tab_question_matching_edit.php?iQues=<?=$value['cou_tab_question_id']?>'&url=<?=base64_encode(getURL())?>'&TB_iframe=true&amp;height=350&amp;width=1000">
	                  <b style="background: none repeat scroll 0 0;color: black;padding: 2px 10px;">Update Câu hỏi này</b>
	               </a>
	            </td>
	            <td align="center"><a onclick="if (confirm('Bạn muốn xóa bản ghi?')){ window.location.href='course_unit_tab_question_matching_delete.php?record_id=<?=$value["cou_tab_question_id"]?>&url=<?=base64_encode($_SERVER['REQUEST_URI'])?>' }" href="#" class="delete"><img border="0" src="<?=$fs_imagepath?>delete.gif"></a></td>
			</tr>
			<? } } ?>
		<? } ?>
	</table>
...
function addblockintab(tab_id){
	...
	$.ajax({
		...
		data:{
			tab_id:tab_id,
			tab_type:"addblockintabquesmatching",
			blockname:blockname
      	},
		url:'ajax.php',
