...
//Call class menu - lay ra danh sach Category
   ...
   $myform->add("test_name", "test_name", 0, 0, "", 1, "Bạn chưa nhập tên của đề thi", 0, "Đề thi này đã có trong cơ sở dữ liệu!");
   	...
   	if($fs_errorMsg == ""){ 	
   		...
   		if($filename != ""){
	         delete_file($fs_table,$id_field,$record_id,"test_image",$imgpath);
   			$myform->add("test_image","filename", 0, 1, "", 0);
         ...
         if($tags != '') save_tags($record_id,$tags,3,2);	
   		//Redirect to:
...
$db_data 	= new db_query("SELECT * FROM test WHERE " . $id_field . " = " . $record_id);
...
<? /*------------------------------------------------------------------------------------------------*/ ?>
   ...
   <?=$form->text("Tên đề thi", "test_name", "test_name", $test_name, "Tên đề thi", 1, 250, "", 255, "", "", "")?>
   <?=$form->select_db_multi("Danh mục", "cat_id", "cat_id", $listAll, "cat_id", "cat_name", $test_cat_id, "Chọn danh mục", 1, "", 1, 0, "", "")?>
