...
$iTyp          = getValue("iTyp","int","GET","3");
$arr_form      = array(3 => "Nhóm 1 (Câu 1 + 2)" , 1 => "Nhóm 2 (Câu 3 + 4)" , 2 => "Nhóm 3 (Câu 5 + 6)");
...
if($iTyp == 1){
   ...
   $myform->add("tec_content", "tec_content", 0, 0, "", 0, "Bạn chưa nhập nội dung cho đoạn văn", 0, "");
...
if($action == "execute"){
   ...
   //$fs_errorMsg .= $upload_ques->show_warning_error();   
 
	//Check form data
	//$fs_errorMsg .= $myform->checkdata();
	//$fs_errorMsg	.= $upload->warning_error;
   //$fs_errorMsg	.= $upload_ques->warning_error;
   ...
   if($total_row < 6){
      ...
      echo("<script>alert('Chỉ được thêm 6 câu trong phần Speaking')</script>");
      redirect($_SERVER['REQUEST_URI']);
   }
...
<? /*------------------------------------------------------------------------------------------------*/ ?>
   ...
   <?if($iTyp == 1){?>
      <?=$form->text("Tiêu đề", "tec_name", "tec_name", "", "Tiêu đề audio", 1, 272, "", 255, "", "", "")?>
      <tr>
         <td></td>
         <td><?=$form->wysiwyg(" Nội dung đoạn văn", "tec_content", $tec_content, "../../resource/wysiwyg_editor/", 800, 250)?></td>
      </tr>
      <?=$form->textarea("Câu hỏi", "tec_ques", "tec_ques", $tec_ques, "Câu hỏi", 0, 272, "", 255, "", "", "")?>
      <?=$form->getFile("Audio câu hỏi", "tec_audio_ques","tec_audio_ques", "Tải audio", 0, 30, "", "")?>
      <tr>
	...
	<?}elseif($iTyp == 2){?>
      ...
      <tr>
         <td>Thời gian audio câu hỏi :</td>
         <td>
            <input id="time_ques_minute" name="time_ques_minute" value="<?=$time_ques_minute?>" type="text" class="time_audio_minute"/> minutes
            <input id="time_ques_second" name="time_ques_second" value="<?=$time_ques_second?>" type="text" class="time_audio_second"/> seconds
            <span style="color: #717F89;">(Ví dụ : mm/ss)</span>
         </td>
      </tr>
      <?=$form->text("Thứ tự", "tec_order", "tec_order", $tec_order, "Thứ tự", 0, 40, "", 255, "", "", "")?>
...
<? /*------------------------------------------------------------------------------------------------*/ ?>
	...
	<table cellpadding="5" cellspacing="0" width="100%" style="border-collapse:collapse;" bordercolor="#CCCCCC" border="1">
		<tr>
         ...
         <th width="200">Audio Question</th>         
         <th width="200">Audio Paragraph</th>
		...
		?>
            ...
            <td align="center" width="200">
               <?
               if($row['tec_audio_ques'] != ""){
                     $url = $data_path.$row['tec_audio_ques'];
