...
*/
$list->add("cv_avatar","Hình ảnh đại diện","string",0,0);
$list->add($name_field,"Tiêu đề","string", 1, 1);
$list->add("cv_price","Giá CV","int",0,0);
$list->add("cat_name","Danh mục","string",0,0);
$list->add("cv_time","Ngày tạo","string",0,0);
$list->add("cv_active","Active","int", 0, 0);
...
$db_listing 	= new db_query("SELECT * FROM cover_letters
                               INNER JOIN categories_multi ON cover_letters.cv_cat_id=categories_multi.cat_id
								 		 WHERE 1".$list->sqlSearch().$sql_filter
									   . " ORDER BY " . $list->sqlSort() . "cv_id DESC "
...
<div id="listing">
   ...
   ?>
      ...
      <td align="center">
		   <img style="height: 40px;" src="<?=$imgpath . "small_" . $row['cv_avatar'] ?>" />
		  </td>
      <td width="250" class="bold" align="center">
         <input type="text" style="width: 240px;color: red;" value="<?=$row[$name_field]?>" />
      </td>
      <td width="250" class="bold" align="center">
         <input type="text" style="width: 240px;color: red;" value="<?=$row['cv_price']?>" />
      </td>
      <td width="200" align="center">
          <input style="width:190px; color: #1C3E7F;" type="text" value="<?=$row['cat_name']?>" />
      </td>
      <td align="center"><?=date("d/m/Y",$row['cv_time'])?></td>
      <?=$list->showCheckbox("cv_active", $row["cv_active"], $row[$id_field])?>
      <?=$list->showEdit($row['cv_id'])?>
      <?=$list->showDelete($row['cv_id'])?>
      <?=$list->end_tr()?>
   <?
     }
   ?>
