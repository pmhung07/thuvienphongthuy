...
checkAddEdit("add");
   ...
   if($iTyp > 0){
      $db_check_type = new db_query("SELECT typ_id FROM type_test WHERE typ_test_id = ". $iTyp ." AND typ_type = 4");
      $total_row = mysql_num_rows($db_check_type->result);
      if($total_row > 0){
         echo("<script>alert('Đã tồn tại phần thi Listening của bài thi này!Hãy chọn đề thi khác!');</script>");
         redirect("add.php");
      }
   }
   
   $arr_get_typ = array();
   $db_select_test = new db_query("SELECT * FROM test STRAIGHT_JOIN type_test ON test_id = typ_test_id WHERE typ_type = 4 GROUP BY typ_test_id ");
   $i = 0;
   ...
   $myform->add("typ_type", "typ_type", 1, 1, 4, 0, "", 0, "");   
      ...
      if($fs_errorMsg == ""){
         ...
         if($filename != ""){
      		...
      		foreach($arr_resize as $type => $arr){
      				resize_image($data_path, $filename, $arr["width"], $arr["height"], $arr["quality"], $type);
      		}
   		}	
...
<? /*------------------------------------------------------------------------------------------------*/ ?>
   ...
   $form->create_table();
	?>
   <?=$form->text_note('<strong style="text-align:center;">----------Thêm mới Test Writing-----------</strong>')?>
   ...
   <?=$form->text("Tên phần thi", "typ_name", "typ_name", "Writing", "Phần thi", 1, 272, "", 255, "", "", "")?>
