...
if($que_id > 0){ 
   ...
   $myform->add("toque_id","que_id",1,1,"",0,"",0,"");
   $myform->add("toque_content","que_content",0,1,"",0,"",0,"");
   $myform->addTable("toeic_questions");
   //Check $action for insert new data
   $fs_errorMsg .= $myform->checkdata();
   if($fs_errorMsg == ""){
      $myform->removeHTML(0);
   	$db_update = new db_execute($myform->generate_update_SQL("toque_id", $que_id));
   	unset($db_update);
...
if($ans_id > 0){
$myform = new generate_form();
$myform->add("totan_id","ans_id",1,1,"",0,"",0,"");
$myform->add("totan_content","ans_content",0,1,"",0,"",0,"");
