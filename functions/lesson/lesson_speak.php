...
function lesson_speak($unit,$unit_num,$unit_name){
    ...
    $rowSpe     = mysql_fetch_assoc($sqlSpe->result);
    ...
    <div class="in_content_v2">
       	...
       	<div class="lesson-content-left">
            ...
            while($rowCtSpea = mysql_fetch_assoc($sqlCtSpea->result)){
            ?>
                <script type="text/javascript">
        ...
        if($myuser->logged == 1){
            ...
            if($chk == 1){
                ...
                $row_own = mysql_fetch_assoc($db_own->result);
                unset($db_own);
                ...
                switch($type){
             	    case "practice":
             	   		if (checkLearn($unit,'writing') == 1) {
             				...
             				$row        = mysql_fetch_assoc($sqlWri->result);
             			...
             			if (checkLearn($unit,'speaking') == 1) {
             				$sqlSpe     = new db_query('SELECT * FROM learn_speaking WHERE learn_unit_id = '.$unit);
             				$row        = mysql_fetch_assoc($sqlSpe->result);
